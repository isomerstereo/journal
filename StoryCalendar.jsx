...
// Configuration mapping states to specific visual assets and handwritten captions
const PANE_PRESETS = {
  OVERWHELMED: {
...
export const StoryCalendar = ({ currentMonth = "June 2026", dailyData = {} }) => {
      ...
      <div className="grid grid-cols-7 gap-2 bg-slate-900/50 p-2 rounded-lg border border-slate-900">
        {daysInMonth.map(day => {
          ...
          return (
            <div 
              key={day} 
              className={`aspect-square border border-slate-800 rounded-md p-1.5 flex flex-col justify-between relative transition-all duration-200 hover:border-slate-600 ${currentPreset.bgClass}`}
            >
              {/* Date & Vitals Dot Indicator Strip */}
              <div className="flex justify-between items-start">
                <span className="text-[10px] font-bold text-slate-500">{day}</span>
                
                {/* Micro High-Density Vitals Ticker (Inspired by image_d1de6c) */}
              ...
              {/* Central Illustration Frame (Route to actual chibi image assets here later) */}
              ...
              <div className="text-[8px] leading-tight text-center truncate text-slate-400 scale-95 origin-bottom">
